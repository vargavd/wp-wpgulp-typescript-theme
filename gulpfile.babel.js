 ...
 * Task: `typescript`.
 *
 * Compile TS files to JS in the same folder. 
 * The options are stored in the tsconfig.json file.
 */
gulp.task("typescript", function () {
    console.log((new Date()).toString());

    var tsResult = gulp.src(config.watchTsFiles, { since: gulp.lastRun('typescript') })
        .pipe(sourcemaps.init())
        .pipe(tsProject());

    return tsResult.js
        .pipe(sourcemaps.write({ sourceRoot: "/wp-content/themes/stbase/assets/ts/" })) // hack: sourceRoot
        .pipe(gulp.dest(config.tsFilesDir));
});
...
gulp.task(
    'default',
    gulp.parallel('typescript', 'styles', /*'vendorsJS', 'customJS',*/ 'images', browsersync, () => {
        gulp.watch(config.watchTsFiles, gulp.series('typescript', reload)); // Reload on TS file changes.
